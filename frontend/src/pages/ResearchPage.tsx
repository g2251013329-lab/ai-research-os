...
const STATUS_STYLES: Record<string, string> = {
  ...
  archived: 'bg-surface-hover text-foreground/55 dark:bg-neutral-800 dark:text-foreground/55',
}
...
export default function ResearchPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-7">
          {[
            ...
            <div key={key} className="rounded-md border border-border bg-surface px-2.5 py-2 text-center dark:border-border dark:bg-surface">
              ...
              <div className="mt-1 text-[10.5px] text-foreground/45">{t(`research.statsKeys.${key}`)}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {view === 'list' && isLoading && <p className="mt-8 text-center text-[13px] text-foreground/45">…</p>}
      {(projects ?? []).length === 0 && !isLoading && (
        <div className="mt-8 rounded-lg border border-dashed border-border p-12 text-center dark:border-border">
          <p className="text-[13px] text-foreground/45">{t('research.empty')}</p>
        </div>
      )}
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {(projects ?? []).map((p) => (
          <button
            ...
            className="group rounded-lg border border-border bg-surface p-4 text-left shadow-card transition-all hover:border-accent hover:shadow-lift dark:border-border dark:bg-surface"
          >
            ...
            <div
              className="mt-1 line-clamp-2 text-[12px] leading-relaxed text-foreground/45"
              data-tip={p.description}
            >
              {p.description || t('research.noDescription')}
            </div>
            <div className="mt-3 flex items-center gap-3 text-[11px] text-foreground/45">
              ...
              <ChevronRight
                size={13}
                className="ml-auto text-foreground/35 transition-transform group-hover:translate-x-0.5 group-hover:text-accent"
              />
            </div>
          </button>
        ))}
